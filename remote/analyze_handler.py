...
def pre_analyze(analyze_id, part_number, workspace, use_cache):
    ...
    with zipfile.ZipFile(zip_file_path) as zip_file:
        ...
        if use_cache:
            list_of_other_files = ['sources-root/compile_command.json',
    ...
    compile_command_path = os.path.join(
        sources_root_path, 'compile_command.json')

    with open(compile_command_path) as compile_command_json:
        try:
            # Assume that there is only one compile command
            compile_command = json.load(compile_command_json)[0]
        except IndexError:
            LOG.error('The compile_command.json was empty.')
        except Exception:
    ...
    if use_cache:
        ...
        with open(cached_files_path) as read_in_cached_files:
            try:
                cached_files = json.load(read_in_cached_files)
            except Exception:
                REDIS_DATABASE.hset(analyze_id, 'state',
                                    AnalyzeStatus.PREANALYZE_FAILED.name)
    ...
    compile_command['directory'] = os.path.join(
        sources_root_path, compile_command['directory'][1:])
    ...
    if 'command' in compile_command:
        ...
        compile_command['arguments'] = compile_command['command']

    modified_compile_command = os.path.join(
        sources_root_path, 'modified_compile_command.json')

    with open(modified_compile_command, 'w') as modified_cdb:
        modified_cdb.write('[' + json.dumps(compile_command) + ']')
...
def analyze(analyze_id, analyze_dir_path):
    ...
    if logging.getLogger().isEnabledFor(logging.DEBUG):
        with open('modified_compile_command.json') as compile_command_json:
            try:
                compile_command = json.load(compile_command_json)
            except Exception:
                REDIS_DATABASE.hset(analyze_id, 'state',
                                    AnalyzeStatus.PREANALYZE_FAILED.name)
                LOG.error('Failed to read in compile command.')

        LOG.debug(compile_command['arguments'])
        LOG.debug(compile_command['directory'])
        LOG.debug(compile_command['file'])

    codechecker_path = os.path.join(
        os.getcwd(), "build/CodeChecker/bin/CodeChecker")

    modified_compile_command = os.path.join(
        analyze_dir_path, 'sources-root', 'modified_compile_command.json')

    command = []
    command.append("%s" % codechecker_path)
    command.append("analyze")
    command.append("%s" % modified_compile_command)
    command.append("-o")
